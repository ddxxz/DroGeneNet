...
for species in ['japonica','indica','zeamays','homo']:#
#species='zeamays'
#species='japonica'
    if species=='zeamays':
   ...
   # treats=['leaf','root']#['CK','CD']#
    ...
    for i,gene_sets in enumerate(all_gene_sets):
        ...
        data=pd.read_csv(f'/home/win/16t1/Deeplearning/data_output/GRN_result/Batch_correction/data/batch_correction_out/expression_DEG/{species}_deg.csv')
        ...
        enr = gp.enrichr(gene_list=gene_list,#所需查询gene_list，可以是一个列表，也可为文件（一列，每行一个基因）
                        ...
                        outdir=f'/home/win/16t1/Deeplearning/data_output/GRN_result/Batch_correction/data/batch_correction_out/expression_GO/{species}',#输出目录
                        top_term=20,
                        cutoff=0.9,#pvalue阈值
                        )
        #ic(enr.results)
        ...
        base_path = f"/home/win/16t1/Deeplearning/data_output/GRN_result/Batch_correction/data/batch_correction_out/expression_GO/funcGO"
        ...
        df.to_csv(f'/home/win/16t1/Deeplearning/data_output/GRN_result/Batch_correction/data/batch_correction_out/expression_GO/{species}_{funcs[i]}_GO.csv',index=False)
