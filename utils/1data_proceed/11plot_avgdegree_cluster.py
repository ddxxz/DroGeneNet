...
for n,speci in enumerate(species):
    ...
    for treat in treats:
        ...
        degree_distributions.append((degrees, degree_freqs,treat))
    #labels=['leaf CK','leaf CD','root CK','root CD']
    labels=['CK','CD']
    axs[i].bar(range(len(avgds)), avgds, align='center',color=['black','red','black','red']) # 假设d是一个字典，转换为值列表进行绘制
    axs[i].set_xticks(range(len(labels)))
    ...
    axs[i].set_xlabel('Group',fontsize=25,weight='bold')
    axs[i].set_ylabel('Avg Degree',fontsize=25,weight='bold')
    axs[i].spines['top'].set_linewidth(1.5)
    axs[i].spines['bottom'].set_linewidth(1.5)
    axs[i].spines['right'].set_linewidth(1.5)
    axs[i].spines['left'].set_linewidth(1.5)
    ...
    if speci=='homo':
        ...
        axs[i+1].set_title(f'{titles[i]}',fontsize=30,weight='bold',pad=15)
        axs[i+1].set_xlabel('Group',fontsize=25,weight='bold')
        axs[i+1].set_ylabel('Cluster Coef',fontsize=25,weight='bold')
        ...
        axs[i+3].set_title(f'{titles[i]}',fontsize=30,weight='bold',pad=15)
        axs[i+3].set_xlabel('Group',fontsize=25,weight='bold')
        axs[i+3].set_ylabel('Cluster Coef',fontsize=25,weight='bold')
        ...
        axs[2].set_ylim(11,20)
        axs[3].set_ylim(0.65,0.7)
        axs[4].set_ylim(0.6,0.8)
        axs[5].set_ylim(0.48,0.65)
        for label in axs[i+3].get_xticklabels() + axs[i+3].get_yticklabels():
            label.set_fontsize(20)
            label.set_weight('bold')
        
    
    for degrees, degree_freqs, treat in degree_distributions:
        axs[i + 6].bar(degrees, degree_freqs, width=0.8, alpha=0.7, label=treat)
    
    axs[i + 6].legend()
    axs[i + 6].set_title(f'{titles[n]}', fontsize=30, weight='bold', pad=15)
    axs[i + 6].set_xlabel('Degree', fontsize=25, weight='bold')
    axs[i + 6].set_ylabel('Frequency', fontsize=25, weight='bold')
    axs[i + 6].spines['top'].set_linewidth(1.5)
    axs[i + 6].spines['bottom'].set_linewidth(1.5)
    axs[i + 6].spines['right'].set_linewidth(1.5)
    axs[i + 6].spines['left'].set_linewidth(1.5)
    axs[i + 6].set_xlim(0,500)
    axs[i + 6].set_ylim(0,200)
    for label in axs[i+6].get_xticklabels() + axs[i+6].get_yticklabels():
        label.set_fontsize(20)
        label.set_weight('bold')
...
plt.tight_layout()
plt.savefig(f'/home/win/16t1/Deeplearning/data_output/GRN_result/Batch_correction/out/expression_WGCNA/WGCNA_degreecluster.png',bbox_inches='tight')
