...
for data_type in data_types:
    if data_type == 'TF':
        allspecies=['wheat','sorghum','homo_C3','homo_C4']
    else:
        allspecies = ['wheat','sorghum','homo_C3','homo_C4','homo']
    for speci in allspecies:#'homo'
        if speci=='zeamays':
            ...
            file_path = '/home/win/4T/GeneDL/OSDrought_GCNAT_Link/data/multispecies/initial_data/wheat/Tae_GO_annotation'
        elif speci=='sorghum':
            file_path = '/home/win/4T/GeneDL/OSDrought_GCNAT_Link/data/multispecies/initial_data/sorghum/Sbi_GO_annotation'
        else:
        ...
        if speci=='zeamays':
            ...
            #GO_term_data['zeaGeneid'] = GO_term_data['Gene_id'].apply(convert_msu_to_rap)
            # 替换 column1, column2, column3 中的基因名
            GO_term_data['taeGeneid'] = GO_term_data['Gene_id'].map(gene_id_map).fillna(GO_term_data['Gene_id'])  # fillna用于保留未找到映射的原始值

            def create_gene_sets(df):
                gene_sets = {}
                grouped = df.groupby('GO_term')
                for name, group in grouped:
                    gene_sets[name] = group['taeGeneid'].tolist()
                return gene_sets
        
        elif speci=='sorghum':
            relation = []
            mapdata = pd.read_csv('/home/win/4T/GeneDL/OSDrought_GCNAT_Link/data/multispecies/initial_data/sorghum/sorgsd_GO_trans.csv')
            # 创建映射字典
            gene_id_map = pd.Series(mapdata['Version1'].values, index=mapdata['Version2.1'].values).to_dict()
            #ic(gene_id_map)
            #GO_term_data['zeaGeneid'] = GO_term_data['Gene_id'].apply(convert_msu_to_rap)
            # 替换 column1, column2, column3 中的基因名
            GO_term_data['sogGeneid'] = GO_term_data['Gene_id'].map(gene_id_map).fillna(GO_term_data['Gene_id'])  # fillna用于保留未找到映射的原始值

            def create_gene_sets(df):
                gene_sets = {}
                grouped = df.groupby('GO_term')
                for name, group in grouped:
                    gene_sets[name] = group['sogGeneid'].tolist()
                return gene_sets
        else:
            relation={}
        ...
        # data_path=f'/home/win/16t1/GeneDL/OSDrought_GCNAT_Link/graphormer2Link_result/graphormer2Link_alltype/EXP_Degree_intersection_DEG/'
        # files=os.listdir(data_path)
        data_path=f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/model_result/louvein_GRNcluster/{data_type}_{speci}/CK-CD/'
        files=os.listdir(data_path)
    ...
    # treats=['leaf','root']#['CK','CD']#
        ...
        for i,gene_sets in enumerate(all_gene_sets):
            ...
            for file in files:
                ...
                # ic(gene_sets)
                # GO富集
                newdir=f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/model_result/louvein_GRNcluster/{data_type}_{speci}/CK-CD_GO/'
                if not os.path.exists(newdir):
                    os.makedirs(newdir)
                if len(gene_list) < 5:
                    df=pd.DataFrame(gene_list)
                    df.to_csv(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/model_result/louvein_GRNcluster/{data_type}_{speci}/CK-CD_GO/{data_type}_{speci}_{funcs[i]}_{module_name}_GO.csv',index=False)
                    continue
                ...
                dot_png = f"exp_{speci}_{funcs[i]}_{title_name}" + "_" + "dot" + ".png"
                #bar_png = f"exp_{species}_{func}" + "_" + "bar" + ".png"
                base_path = f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/model_result/louvein_GRNcluster/{data_type}_{speci}/CK-CD_GO/funcGO'
                ...
                df= dotplot(enr.results,
                        ...
                        #figsize=(3,5),
                        xticklabels_rot=45,
                        title=f'GO Top15 Pathway DEG {speci} {funcs[i]}', 
                        show_ring=True,
                        #marker='o',
                        cmap='RdBu',
        ...
        data_path=f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/model_result/louvein_GRNcluster/{data_type}_{speci}/CK-CE/'
        files=os.listdir(data_path)
    ...
    # treats=['leaf','root']#['CK','CD']#
        ...
        for i,gene_sets in enumerate(all_gene_sets):
            ...
            for file in files:
                ...
                # ic(gene_sets)
                # GO富集
                newdir=f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/model_result/louvein_GRNcluster/{data_type}_{speci}/CK-CE_GO/'
                if not os.path.exists(newdir):
                    os.makedirs(newdir)
                if len(gene_list) < 5:
                    df.to_csv(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/model_result/louvein_GRNcluster/{data_type}_{speci}/CK-CE_GO/{data_type}_{speci}_{funcs[i]}_{module_name}_GO.csv',index=False)
                    continue
                title_name=file.split('.')[0]
                enr = gp.enrichr(gene_list=gene_list,#所需查询gene_list，可以是一个列表，也可为文件（一列，每行一个基因）
                                gene_sets= gene_sets,#['GO_Biological_Process_2018'],#,#gene set library，多个相关的gene set 。如所有GO term组成一个gene set library.
                ...
                dot_png = f"exp_{speci}_{funcs[i]}_{title_name}" + "_" + "dot" + ".png"
                #bar_png = f"exp_{species}_{func}" + "_" + "bar" + ".png"
                base_path = f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/model_result/louvein_GRNcluster/{data_type}_{speci}/CK-CE_GO/funcGO'
