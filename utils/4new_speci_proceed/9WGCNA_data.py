...
all_species=['wheat','indica_BGI','japonica','zeamays','sorghum']  #'homo','homo_C3','homo_C4'
for species in all_species:

    with open(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/data/batch_correction_out/count/{species}_label_sample_count.pkl','rb') as fp:
        data=pickle.load(fp)
    ic(data.shape)
    #ic(data.iloc[0,:])
    DEG_data = pd.read_csv(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/data/batch_correction_out/expression_DEG/CK_CD/{species}/{species}_CK_CD_DEG.csv')
    DEG = set(DEG_data['FEATURE_ID'])
    #ic(DEG)
    first_row = data.iloc[[0]]  # Retain the first row
    filtered_data = data.iloc[1:][data.iloc[1:].index.isin(DEG)]  # Filter rows from the second row onwards
    data = pd.concat([first_row, filtered_data])

    first_row = data.iloc[0:1, :]  # CK/CD的行
    gene_ids = data.iloc[1:, 0]  # 基因ID列
    numeric_data = data.iloc[1:, 1:].apply(pd.to_numeric, errors='coerce')
    numeric_data = np.log2(numeric_data + 1)
    numeric_data = numeric_data.fillna(0)
    numeric_data.insert(0, data.columns[0], gene_ids)
    final_data = pd.concat([first_row, numeric_data])
    data = final_data
    ic(data)
    ic(data.shape)
    ...
    data = data[1:]
    # if species =='wheat':
    #     negative_ratio = (data < 0).mean(axis=1)
    #     data = data[negative_ratio <= 1/250]
    # 重置索引
    ...
    for i,matrix in enumerate([df_ck,df_cd]):  #,df_ce
        ...
        matrix.to_csv(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/data/batch_correction_out/expression_WGCNA/{species}_{treat[i]}_WGCNA.txt',sep='\t')
