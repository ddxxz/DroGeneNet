...
species=['homo','homo_C3','homo_C4']#"indica","indica_BGI",'japonica','wheat','zeamays','sorghum',
#organs=['leaf','root']
#for organ in organs:
for speci in species:
    ...
    if speci == 'homo' or speci == 'homo_C3' or speci == 'homo_C4':
        pass
    else:
    ...
    directory=f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/data/batch_correction_out/expression_DEG/CK_CD/{speci}/'
    if not os.path.exists(directory):
        os.makedirs(directory)
    combined_df_ck_cd = combined_df_ck_cd.drop_duplicates()
    ...
    # if speci =='wheat':
    #     selected_genes_up = df[(df['qvalue'] < 0.05) & (df['log2FC'] > 0.45)] #1.3倍  # 1.5倍 0.585
    #     selected_genes_up.to_csv(f'{directory}/{speci}_CK_CD_DEG_up.csv')
    #     selected_genes_down = df[(df['qvalue'] < 0.05) & (df['log2FC'] < -0.45)] #1.3倍 #1.5倍 0.585
    ...
    selected_genes_down = selected_genes_down.drop_duplicates()
    selected_genes_down.to_csv(f'{directory}/{speci}_CK_CD_DEG_down.csv')
    ...
    sorted_selected_genes_up = sorted_selected_genes_up.drop_duplicates()
