...
file_path1 = '/home/win/4T/GeneDL/OSDrought_GCNAT_Link/data/multispecies/initial_data/sorghum/regulation_merged_Sbi.txt'  # 这里替换为你的文本文件路径
file_path2 = '/home/win/4T/GeneDL/OSDrought_GCNAT_Link/data/multispecies/initial_data/sorghum/Sbi_TF_list.txt'  # 这里替换为你的文本文件路径
...
# 保存到CSV文件
pd.DataFrame(unique_values).to_csv('/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/data/batch_correction_data/TF/sorghum/genes.csv', index=False)  # 第一列的数据保存为 column1.csv

files3 = '/home/win/4T/GeneDL/OSDrought_GCNAT_Link/data/multispecies/initial_data/sorghum/sorgsd_GO_trans.csv'
df3 = pd.read_csv(files3)
data = []

# 创建映射字典
gene_id_map = pd.Series(df3['Version1'].values, index=df3['Version2.1'].values).to_dict()
...
regulation.to_csv('/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/data/batch_correction_data/TF/sorghum/TF_Target.csv',index=False)

TF_list = pd.concat([column3_mapped, df2.iloc[:, 2]],axis=1)
TF_list.columns = ['TF','Family']
TF_list.to_csv('/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/data/batch_correction_data/TF/sorghum/TF_family.csv',index=False)
...
df = pd.read_csv("/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/data/batch_correction_data/TF/sorghum/TF_Target.csv")
...
#ic(gene_index)
import json
filename = "/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/data/batch_correction_data/TF/sorghum/gene_index.json"
...
ic(len(merged_df))
merged_df.to_csv(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/data/batch_correction_data/TF/sorghum/ALL_Genefile.csv')



#print(target)
pd.DataFrame({'TF': list(dict.fromkeys(selected_rows['TF'])), 'index': list(dict.fromkeys(tf))}).to_csv(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/data/batch_correction_data/TF/sorghum/TF.csv')
pd.DataFrame({'Gene': list(dict.fromkeys(selected_rows['Target'])), 'index': list(dict.fromkeys(target))}).to_csv(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/data/batch_correction_data/TF/sorghum/Target.csv')

pd.DataFrame(unique_genes).to_csv(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/data/batch_correction_data/TF/sorghum/unique_genes.csv', index=False)
# 保存转换后的索引对索引的文件

ic(data['TF'].max(),data['Target'].max())
data.to_csv(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/data/batch_correction_data/TF/sorghum/index_mapped_TF_Target.csv', index=False)
...
install()
# 读取已有的CSV文件
df = pd.read_csv(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/data/batch_correction_data/TF/sorghum/index_mapped_TF_Target.csv')
gene_file = pd.read_csv(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/data/batch_correction_data/TF/sorghum/ALL_Genefile.csv')
...
# 保存为新的CSV文件
all_samples.to_csv(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/data/batch_correction_data/TF/sorghum/all_labels.csv',index=False)

# # #--------------------------------------444保存uniquegene与网络筛选的表达数据---------------------------------------------------2086 第三步
import pickle
# import pandas as pd
# 读取unique gene的CSV文件
unique_gene_df = pd.read_csv(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/data/batch_correction_data/TF/sorghum/unique_genes.csv')
...
# 保存筛选后的结果为新的CSV文件
filtered_expression_data_df.to_pickle('/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/data/batch_correction_data/TF/sorghum/ExpressionData_unique_networkfilter.pkl')
