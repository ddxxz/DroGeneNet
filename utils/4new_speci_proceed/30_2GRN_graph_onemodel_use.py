...
def get_GRN(condition,speci,data_type):
    ...
    data['TF'] = data['TF'].astype(str)
    data['Target'] = data['Target'].astype(str)
    data['tf_family'] = data['tf_family'].astype(str)
    data['target_family'] = data['target_family'].astype(str)
    gene_family_data =pd.read_csv(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/gene_family/{speci}_genefamily.tsv',sep='\t')
    gene_family_data['To'] = gene_family_data['To'].astype(str)
    family_dict = gene_family_data.set_index('To')['Family Membership'].to_dict()
    # 更新 tf_family 和 target_family 列
    data['tf_family'] = data.apply(
        lambda row: family_dict.get(row['TF'], row['tf_family']) if row['tf_family'] == 'target_gene' else row['tf_family'],
        axis=1
    )
    data['target_family'] = data.apply(
        lambda row: family_dict.get(row['Target'], row['target_family']) if row['target_family'] == 'target_gene' else row['target_family'],
        axis=1
    )
    ic(set(data['target_family']))
