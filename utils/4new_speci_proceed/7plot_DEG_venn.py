...
for i,name in enumerate(['DEG','DEG_up','DEG_down']):
    ...
    indica_DEG_data = pd.read_csv(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/data/batch_correction_out/expression_DEG/CK_CD/indica_BGI/indica_BGI_CK_CD_{name}.csv')
    japonica_DEG_data = pd.read_csv(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/data/batch_correction_out/expression_DEG/CK_CD/japonica/japonica_CK_CD_{name}.csv')
    zeamays_DEG_data = pd.read_csv(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/data/batch_correction_out/expression_DEG/CK_CD/zeamays/zeamays_CK_CD_{name}.csv')
    wheat_DEG_data = pd.read_csv(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/data/batch_correction_out/expression_DEG/CK_CD/wheat/wheat_CK_CD_{name}.csv')
    sorghum_DEG_data = pd.read_csv(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/data/batch_correction_out/expression_DEG/CK_CD/sorghum/sorghum_CK_CD_{name}.csv')


    indica_DEG = indica_DEG_data['FEATURE_ID']
    japonica_DEG = japonica_DEG_data['FEATURE_ID']
    zeamays_DEG = zeamays_DEG_data['FEATURE_ID']
    wheat_DEG = wheat_DEG_data['FEATURE_ID']
    sorghum_DEG = sorghum_DEG_data['FEATURE_ID']
    ...
    plt.savefig(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/homo_{name}.png',bbox_inches='tight')
    plt.clf()
    if name == 'DEG':
