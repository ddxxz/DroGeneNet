...
## 打开多线程
enableWGCNAThreads()
speci_list <- c('sorghum')  #'indica_BGI','japonica','wheat','zeamays',,'homo','homo_C3','homo_C4'  
treat_list <- c('CK', 'CD')
...
for (speci in speci_list) {
  for (treat in treat_list) {
    ...
    # Rfile_path <- sprintf("/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/expression_WGCNA/deg/%s_%s_WGCNA.RData", speci, treat)
    ...
    dynamicMods = cutreeDynamic(dendro = geneTree, distM = dissTOM, deepSplit = 2, cutHeight = 0.95,pamRespectsDendro = FALSE, minClusterSize = minModuleSize)
    table(dynamicMods)
    ...
    save(MEs, moduleLabels, moduleColors,dynamicColors,dynamicMods,geneTree,dissTOM,TOM,file = sprintf("/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/expression_WGCNA/deg/%s_%s_WGCNA.RData",speci,treat))

    load(sprintf("/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/expression_WGCNA/deg/%s_%s_WGCNA.RData",speci,treat))
    #par(family="Times New Roman", cex=2)
    #png(sprintf("/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/expression_WGCNA/deg/%s_%s_WGCNA.png",speci,treat), width = 500, height = 300)  # 640x480 pixels
    pdf(sprintf("/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/paper_plot/py_5speci/%s_%s_WGCNA.pdf", speci, treat), width = 7, height = 5)  # 这里的宽度和高度可以根据需要调整
    ...
    dev.off()
        ...
        # 保存路径和文件名
    pdf(file = sprintf("/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/paper_plot/py_5speci/%s_%s_TOM.pdf", speci, treat), width = 8, height = 8)
    #png(sprintf("/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/paper_plot/py_5speci/%s_%s_TOM.png",speci,treat), width = 500, height = 300)  # 640x480 pixels

    # 绘制 TOM 热图
    # 参数解释：
    # geneTree: 基因层次聚类树
    # dissTOM: 拓扑重叠矩阵的距离 (1 - TOM)
    # moduleColors: 模块颜色，用于在热图中显示模块信息
    # main: 主标题
    # CairoPDF(file = sprintf("/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/paper_plot/py_5speci/%s_%s_TOM.pdf", speci, treat), 
    #     width = 8, height = 6)
    # par(mar = c(5, 5, 2, 2))  # 设置边距: 底部、左侧、顶部、右侧
    # layout(matrix(c(1,2), ncol=2), widths=c(4,1)) # 保持布局
    ...
    #selectColors, 
    moduleColors,
    #main = "Customized TOM Heatmap",
    #colorRampPalette = myheatcol#colorRampPalette(c("green", "white", "purple"))
    )
    # par(mar=c(5,1,5,3))
    # image(1, seq(0, 1, length=250), matrix(seq(0, 1, length=250), ncol=1), 
    #       col = myheatcol, xlab = "", ylab = "TOM Value", axes=FALSE)
    ...
    # 输出为CSV文件
    write.csv(combinedData, file = sprintf("/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/expression_WGCNA/all/%s_%s_WGCNA.csv",speci,treat), row.names = FALSE, quote = FALSE)
  }
}
#基因过滤
# datExpr1<-datExpr2
