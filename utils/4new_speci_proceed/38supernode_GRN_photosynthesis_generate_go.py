...
for data_type in data_types:
    ...
    for speci in species:
        data_CD=pd.read_csv(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/model_result/sub_GO_module/{data_type}_{speci}_CD_gene_photosynthesis_rows.csv')
        ...
        filtered_df = data_CD#data_CD[(data_CD['TF'].isin(deg_gene) & data_CD['Target'].isin(deg_gene))]
        ic(len(filtered_df))
        #ic(filtered_df.keys())
        ...
        if len(target_nodes.keys())==0:
            continue
        else:
            pd.DataFrame(target_nodes.keys()).to_csv(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/model_result/supernode_GRN_photosynthesis/{data_type}_{speci}_CK_CD_go.csv',index=False)


    for speci in species:
        data_CD=pd.read_csv(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/model_result/sub_GO_module/{data_type}_{speci}_CE_gene_photosynthesis_rows.csv')
        ...
        if len(target_nodes.keys())==0:
            ...
            pd.DataFrame(target_nodes.keys()).to_csv(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/model_result/supernode_GRN_photosynthesis/{data_type}_{speci}_CK_CE_go.csv',index=False)
    ...
    for speci in species:
        for treat in ['CK_CD','CK_CE']:
            ic(data_type,speci,treat)
            file_path = f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/model_result/supernode_GRN_photosynthesis/{data_type}_{speci}_{treat}_go.csv'
            ...
            mapped_values.to_csv(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/model_result/supernode_GRN_photosynthesis/{data_type}_{speci}_{treat}_go_class.csv',index=False,header=False)
...
#==============================绘图====================================
    for speci in species:
        ...
        plt.savefig(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/model_result/supernode_GRN_photosynthesis/{data_type}_{speci}_CK_CD.png', bbox_inches='tight')
        plt.clf()
    ...
    for speci in species:
        ...
        #ic(filtered_df.keys())
        #ic(filtered_df['target_family'])
        category_file = f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/model_result/supernode_GRN_photosynthesis/{data_type}_{speci}_CK_CE_go_class.csv'
        if os.path.exists(category_file):
            # 文件存在时，读取并处理数据
            category_numbers = pd.read_csv(category_file, header=None).squeeze().tolist()
        else:
            continue
        #ic(category_numbers)
        #数据预处理
        ...
        #plt.legend(scatterpoints=1, loc='upper left', fontsize=10, markerscale=0.1, labelspacing=0.5, handletextpad=0.5)
        plt.title(f'Super GRN for {speci}',fontsize=30,fontweight='bold')
        plt.savefig(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/model_result/supernode_GRN_photosynthesis/{data_type}_{speci}_CK_CE.png', bbox_inches='tight')
        plt.clf()
