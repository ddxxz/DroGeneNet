import gseapy as gp
import os
from plot_go import barplot, dotplot
import pandas as pd
...
#高粱geneid转换 https://ngdc.cncb.ac.cn/sorgsd/geneconversion
#-------------------------------获取Geneid和GOterm---------------------------------------------------
# 假设文件是以制表符分隔的，将 'sep' 参数替换成 ',' 如果是逗号分隔的CSV
for species in ['homo','homo_C3','homo_C4']:#'japonica','indica','wheat','zeamays','sorghum',
#species='zeamays'
#species='japonica'
    if species=='zeamays':
   ...
   # treats=['leaf','root']#['CK','CD']#
    ...
    for i,gene_sets in enumerate(all_gene_sets):
        ...
        data=pd.read_csv(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/data/batch_correction_out/expression_DEG/CK_CD/{species}/{species}_CK_CD_DEG.csv')

        gene_list = data['FEATURE_ID']
        if species == 'sorghum':
            ...
            # 创建映射字典
            gene_id_map = pd.Series(mapdata['new_column'].values, index=mapdata['Protein stable ID'].values).to_dict()
            #ic(gene_id_map)
            #GO_term_data['zeaGeneid'] = GO_term_data['Gene_id'].apply(convert_msu_to_rap)
            # 替换 column1, column2, column3 中的基因名
            gene_list = gene_list.map(gene_id_map)
        ...
        enr = gp.enrichr(gene_list=gene_list,#所需查询gene_list，可以是一个列表，也可为文件（一列，每行一个基因）
                        ...
                        outdir=f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/data/batch_correction_out/expression_GO/{species}',#输出目录
                        top_term=20,
                        cutoff=0.9,#pvalue阈值
                        )
        #ic(enr.results)
        #all_results.append(enr.results)
        dot_png = f"exp_{species}_{funcs[i]}" + "_" + "dot" + ".png"
        #bar_png = f"exp_{species}_{func}" + "_" + "bar" + ".png"
        base_path = f"/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/data/batch_correction_out/expression_GO/funcGO"
        ...
        df.to_csv(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/data/batch_correction_out/expression_GO/{species}_{funcs[i]}_GO.csv',index=False)
