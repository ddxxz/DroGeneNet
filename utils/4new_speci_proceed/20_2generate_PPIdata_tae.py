...
with open('/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/data/batch_correction_data/PPI/wheat/wheat_PPI_gene.pkl', 'rb') as file:
    # 使用 pickle 加载数据
    df = pickle.load(file)
ic(df)
...
#ic(gene_index)
import json
filename = "/home/win/4T/GeneDL//data_output/GRN_result/Batch_correction/data/batch_correction_data/PPI/wheat/gene_index.json"
...
ic(len(merged_df))
merged_df.to_csv(f'/home/win/4T/GeneDL//data_output/GRN_result/Batch_correction/data/batch_correction_data/PPI/wheat/ALL_Genefile.csv')



#print(protein2)
pd.DataFrame({'protein1': list(dict.fromkeys(selected_rows['protein1'])), 'index': list(dict.fromkeys(protein1))}).to_csv(f'/home/win/4T/GeneDL//data_output/GRN_result/Batch_correction/data/batch_correction_data/PPI/wheat/protein1.csv')
pd.DataFrame({'protein2': list(dict.fromkeys(selected_rows['protein2'])), 'index': list(dict.fromkeys(protein2))}).to_csv(f'/home/win/4T/GeneDL//data_output/GRN_result/Batch_correction/data/batch_correction_data/PPI/wheat/protein2.csv')

pd.DataFrame(unique_genes).to_csv(f'/home/win/4T/GeneDL//data_output/GRN_result/Batch_correction/data/batch_correction_data/PPI/wheat/unique_genes.csv', index=False)
# 保存转换后的索引对索引的文件

#ic(data['PROTEIN1'].max(),data['Protein2'].max())
data.to_csv(f'/home/win/4T/GeneDL//data_output/GRN_result/Batch_correction/data/batch_correction_data/PPI/wheat/index_mapped_protein1_protein2.csv', index=False)
...
# 读取已有的CSV文件
df = pd.read_csv(f'/home/win/4T/GeneDL//data_output/GRN_result/Batch_correction/data/batch_correction_data/PPI/wheat/index_mapped_protein1_protein2.csv')
ic(df.shape)
...
gene_file = pd.read_csv(f'/home/win/4T/GeneDL//data_output/GRN_result/Batch_correction/data/batch_correction_data/PPI/wheat/ALL_Genefile.csv')
...
ic(len(existing_index_pairs))

# 生成负样本
def generate_negative_samples(protein1_unique_values, existing_index_pairs, sample_size_multiplier=3):
    negative_samples = []

    # 按批次处理，避免一次性处理所有组合
    for protein1 in tqdm(protein1_unique_values, desc="Generating negative samples"):
        potential_combinations = [(protein1, i) for i in range(len(gene_file)) if (protein1, i) not in existing_index_pairs and i != protein1]
        
        # 随机选择若干个负样本组合
        sample_size = min(len(potential_combinations), sample_size_multiplier * len(existing_index_pairs) // len(protein1_unique_values))
        selected_negative_samples = random.sample(potential_combinations, sample_size)
        negative_samples.extend(selected_negative_samples)

        # 当负样本达到一定数量时返回（逐批生成）
        if len(negative_samples) >= 10000:
            yield pd.DataFrame(negative_samples, columns=['protein1', 'protein2'])
            negative_samples = []

    if negative_samples:
        yield pd.DataFrame(negative_samples, columns=['protein1', 'protein2'])

# 构建 DataFrame 来存储正负样本
all_samples = pd.DataFrame(columns=['protein1', 'protein2', 'Label'])

# 将正样本添加到 DataFrame
positive_samples = existing_pairs.copy()
positive_samples['Label'] = 1
all_samples = pd.concat([all_samples, positive_samples], ignore_index=True)

# 生成负样本并逐批处理，不使用中间文件
for negative_batch in generate_negative_samples(protein1_unique_values, existing_index_pairs):
    negative_batch['Label'] = 0
    all_samples = pd.concat([all_samples, negative_batch], ignore_index=True)

# 保存最终结果为新的CSV文件
all_samples.to_csv(f'/home/win/4T/GeneDL//data_output/GRN_result/Batch_correction/data/batch_correction_data/PPI/wheat/all_labels.csv', index=False)

ic(all_samples.shape)
...
# 读取unique gene的CSV文件
unique_gene_df = pd.read_csv(f'/home/win/4T/GeneDL//data_output/GRN_result/Batch_correction/data/batch_correction_data/PPI/wheat/unique_genes.csv')
...
# 保存筛选后的结果为新的CSV文件
filtered_expression_data_df.to_pickle('/home/win/4T/GeneDL//data_output/GRN_result/Batch_correction/data/batch_correction_data/PPI/wheat/ExpressionData_unique_networkfilter.pkl')
