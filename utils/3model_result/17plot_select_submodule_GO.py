...
for data_type in data_types:
    ...
    for speci in species:
        for treat in ['CK-CD','CK-CE']:
            csv_files = [f for f in os.listdir(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/model_result/louvein_GRNcluster/{data_type}_{speci}/{treat}_GO/') if f.endswith('.csv')]
            csv_file_count = len(csv_files)//3
            for module in range(csv_file_count):
                if not os.path.exists(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/model_result/louvein_GRNcluster/{data_type}_{speci}/{treat}_GO/{data_type}_{speci}_BP_{module}_GO.csv'):
                    continue
                else:
                    bp_data = pd.read_csv(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/model_result/louvein_GRNcluster/{data_type}_{speci}/{treat}_GO/{data_type}_{speci}_BP_{module}_GO.csv')
                    cc_data = pd.read_csv(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/model_result/louvein_GRNcluster/{data_type}_{speci}/{treat}_GO/{data_type}_{speci}_CC_{module}_GO.csv')
                    mf_data = pd.read_csv(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/model_result/louvein_GRNcluster/{data_type}_{speci}/{treat}_GO/{data_type}_{speci}_MF_{module}_GO.csv')

                    combined_df = pd.concat([bp_data, mf_data, cc_data], ignore_index=True)#
                    if 'P-value' not in combined_df.columns:
                        continue
                    ic(f'{data_type}_{speci}/{treat}',combined_df)
                    # 定义处理和绘图的函数
                    def process_and_plot(data, title):
                        # 筛选Adjusted P-value小于0.05的行
                        ...
                        #filtered_data.to_csv(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/model_result/louvein_GRNcluster/comprehensive_GO/{data_type}_{speci}_{treat}_{module}_select_GO.csv',index=False)
                        filtered_data = pd.read_csv(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/model_result/louvein_GRNcluster/comprehensive_GO/{data_type}_{speci}_{treat}_{module}_select_GO.csv')
                        # 设置横坐标为Overlap中的分数，这里我们将它转换为整数分子以供后续使用
                        richfactor = filtered_data['Overlap']
                        richfactor_percent = richfactor.apply(lambda x: float(x.split('/')[0]) / float(x.split('/')[1]) * 100 if '/' in x else None)
                        numerators = richfactor.apply(lambda x: int(x.split('/')[0]) if '/' in x and x.split('/')[0].isdigit() else None)
                        ...
                        # 显示图形
                        plt.savefig(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/model_result/louvein_GRNcluster/comprehensive_GO/{data_type}_{speci}_{treat}_{module}.png', bbox_inches='tight')
