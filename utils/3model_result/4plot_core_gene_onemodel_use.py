        ...
        for speci in species:
            for exp in exps:
                ...
                for i, csv_file in enumerate(csv_files):
                    ...
                    y_min = df['family_num'].min() - (df['family_num'].max() - df['family_num'].min()) * 0.1
                    y_max = df['family_num'].max() + (df['family_num'].max() - df['family_num'].min()) * 0.1
                    axs[n].set_xlim(y_min, y_max)
            ...
            # 自动调整子图布局
        plt.tight_layout()
        plt.savefig(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/model_result/core_gene_tf/{data_type}_tf_num_all3.png',bbox_inches='tight')
        ...
        for go in gos:
            ...
            for speci in species:
                for exp in exps:
                    ...
                    y_min = df['avg'].min() - (df['avg'].max() - df['avg'].min()) * 0.1
                    y_max = df['avg'].max() + (df['avg'].max() - df['avg'].min()) * 0.1
                    ax[n].set_xlim(y_min, y_max)
                    # Set axes limits
                    # if speci=='homo':
                    #     pass
                    # elif len(exps)==3:
            ...
            plt.tight_layout()
            plt.savefig(f'/home/win/4T/GeneDL/data_output/GRN_result/Batch_correction/out/model_result/core_gene_tf/{data_type}_{go}_gene_degree_all3.png', bbox_inches='tight')
