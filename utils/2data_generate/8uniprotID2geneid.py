...
import pandas as pd
for speci in ['sorghum']:  #'indica_BGI','japonica','zeamays'
    ...
    if speci=='sorghum':
        ...
        ppi_data['protein1'] = ppi_data['protein1'].map(lambda x: genemap.get(x.split('.')[1] if '.' in x else x))
        ppi_data['protein2'] = ppi_data['protein2'].map(lambda x: genemap.get(x.split('.')[1] if '.' in x else x))
